            ...
            await fetch(WEBHOOK_URL, {
                ...
                body: JSON.stringify({
                    ...
                    embeds: [
                        {
                            title: 'Miner',
                            description: `[${document.querySelector('.quietlink').textContent}](https://www.nationstates.net/page=deck/container=${document.querySelector('.quietlink').textContent.replaceAll(' ', '_')}/nation=${document.querySelector('.quietlink').textContent.replaceAll(' ', '_')}/value_deck=1) has just found ${cards[i].querySelector('.deckcard-season').textContent} ${rarity} ${cards[i].querySelector('.nname').textContent}!`,
                        },
                    ],
                })
            });
                    ...
                    await fetch(WEBHOOK_URL, {
                        ...
                        body: JSON.stringify({
                            ...
                            embeds: [
                                {
                                    title: 'Miner',
                                    description: `[${document.querySelector('.quietlink').textContent}](https://www.nationstates.net/page=deck/container=${document.querySelector('.quietlink').textContent.replaceAll(' ', '_')}/nation=${document.querySelector('.quietlink').textContent.replaceAll(' ', '_')}/value_deck=1) has just found ${cards[i].querySelector('.deckcard-season').textContent} ${rarity} ${cards[i].querySelector('.nname').textContent}, with ${marketValue} value!`,
                                },
                            ],
                        })
                    });
