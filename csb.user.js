...
// @downloadURL  https://github.com/Kractero/userscripts/raw/main/csb.user.js
// @updateURL    https://github.com/Kractero/userscripts/raw/main/csb.user.js
// @grant        none
// ==/UserScript==
...
(function () {
  ...
  if (localStorage.getItem('buttonshow') === 'true')
  ...
  let dataArray =
    puppetList && puppetList.length > 0 ? puppetList.split('\n') : [];
...
`;
    ...
    document.getElementById('x-rces-cp-onestep-form').onsubmit = function () {
      ...
      document.getElementById('x-rces-cp-confirm-pass').value =
        document.getElementById('x-rces-cp-pass').value;
      return true;
    };
  ...
  if (
    ...
    window.location.href.includes(`/nation=${nation}/page=deck/`)
  ) {
    ...
    if (!window.location.href.includes('card')) {
      ...
      gift.addEventListener('click', () => {
        junks[currentJunk]
          .querySelector('.deckcard-info-cardbuttons a:last-child')
          .click();
      });
    }
  } else {
  ...
  if (
    window.location.href.includes('https://www.nationstates.net/page=deck/card')
  ) {
