...
;(function () {
  ...
  // let ask_match = document.querySelectorAll('.cardauctionunmatchedrow-bid .cardprice')[document.querySelectorAll('.cardauctionunmatchedrow-bid .cardprice').length-1].textContent
  ...
  hotkeys(`b,shift+b`, function (event, handler) {
    event.preventDefault()
  ...
  cards.forEach(card => {
    ...
    const cardTitle = card.querySelector('.rlink').textContent
  ...
  hotkeys(`j,num_5`, function (event, handler) {
    event.preventDefault()
  ...
  if (window.location.href.indexOf('/gift=1') > -1) {
    const check = document.querySelector('.deckcard-category')
